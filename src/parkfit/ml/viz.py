...
def occupancy_heatmap(profile, label: str = "", out_dir: Path | None = None):
    ...
    ax.grid(False)
    style(ax, title=f"Occupancy by weekday and hour{f': {label}' if label else ''}", xlabel="hour")
...
def lambda_accuracy(estimated: np.ndarray, truth: np.ndarray, out_dir: Path | None = None):
    ...
    style(
        ax,
        title="Decay rate: estimated against truth",
        xlabel="true lambda (per minute)",
        ylabel="estimated lambda (per minute)",
    )
...
def sampling_cost(recovered: dict[int, float], out_dir: Path | None = None):
    ...
    ax.annotate(
        "one-minute sensor", (0.02, 0.93), xycoords="axes fraction", color=INK_SOFT, fontsize=9
    )
    ax.set_ylim(0, 118)
    style(
        ax,
        title="Fraction of the true decay rate a polling interval recovers",
        xlabel="polling interval (minutes)",
        ylabel="recovered (%)",
    )
    fig.tight_layout()
...
def feature_importance(importance: dict[str, float], top: int = 10, out_dir: Path | None = None):
...
def calibration(
    probabilities: np.ndarray, labels: np.ndarray, bins: int = 10, out_dir: Path | None = None
):
    ...
    ax.plot(
        [0, 1], [0, 1], color=INK_FAINT, linewidth=1.2, linestyle="--", label="perfect calibration"
    )
    ax.plot(
        centres, observed, color=SERIES[0], linewidth=2, marker="o", markersize=7, label="model"
    )
    ...
    style(
        ax,
        title="Calibration",
        xlabel="predicted probability occupied",
...
def target_heatmap(
    heatmap: np.ndarray, image: np.ndarray | None = None, out_dir: Path | None = None
):
    ...
    ax.grid(False)
    ax.set_title(
        "Centre heatmap target (all classes collapsed)",
        color=INK,
        fontsize=11,
        fontweight="bold",
        loc="left",
...
def gaussian_radius_curve(out_dir: Path | None = None):
    ...
    style(
        ax,
        title="Gaussian splat radius grows with the object",
        xlabel="box width (grid cells)",
        ylabel="radius (cells)",
    )
    fig.tight_layout()
...
def grid_of_scenes(
    images: np.ndarray, labels: list[list[dict]], columns: int = 3, out_dir: Path | None = None
):
    ...
    for index, ax in enumerate(flat):
        ...
        ax.set_title(f"{len(labels[index])} vehicles", color=INK_SOFT, fontsize=9, loc="left")
